...
def compute_text(
    ...
    print('avg ulip text score:', sum(scores)/len(scores))
    print('count: ' + str(len(scores)))
    avg_ulip = sum(scores)/len(scores)
    return avg_ulip, len(scores)
...
def test_text(path_list, output_path):
    data_dir = 'example_data/testset/testset_v1_1_12_merged_catetory'
    avg_ulip_list = []
    for path in path_list:
        avg_ulip, sum_num = compute_text(data_dir=data_dir, output_dir=path)
        avg_ulip_list.append([avg_ulip, sum_num])
    res_dict = dict(zip(path_list, avg_ulip_list))

    import json
    # 打开一个文件以写入模式
    with open(os.path.join(output_path, 'avg_ulip_text.json'), 'w', encoding='utf-8') as file:
...
def test_image(path_list, output_path):
    ...
    avg_ulip_list = []
    for path in path_list:
        avg_ulip, sum_num = compute_image(data_dir=data_dir, output_dir=path)
        avg_ulip_list.append([avg_ulip, sum_num])
    res_dict = dict(zip(path_list, avg_ulip_list))

    import json
    # 打开一个文件以写入模式
    with open(os.path.join(output_path, 'avg_ulip_image.json'), 'w', encoding='utf-8') as file:
        # 使用json.dump()方法将字典写入文件
        json.dump(res_dict, file, ensure_ascii=False, indent=4)
    print('Finished!')
...
if __name__ == '__main__':
    ...
    args.is_test_all = False
    args.is_test_image = False
